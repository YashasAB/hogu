...
router.put("/matches/:matchId/interest", requireAdminAuth, async (req: any, res: any) => {
  try {
    const { matchId } = req.params;
...
router.get("/matches/export/csv", requireAdminAuth, async (req: any, res: any) => {
  try {
    const matches = await prisma.datingMatch.findMany({
      orderBy: { created_at: "desc" },
      include: {
    ...
    const allAvailability = matches.flatMap((m) =>
...
router.get("/users/:userId/get-to-know", requireAdminAuth, async (req: any, res: any) => {
  try {
    const { userId } = req.params;
