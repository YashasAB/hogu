...
export default function RestaurantAdminPanel() {
  ...
  const addSlots = async () => {
    try {
      const result = await api.addSlots({ date, start, end, interval, capacity });
            ...
            <section className="rounded-2xl p-6 ring-1 ring-white/10 bg-slate-900/70 mb-8">
              ...
              {loading ? (
                ...
                <div className="mt-4 space-y-2">
                  {slots.map((slot) => (
                    <div key={slot.id} className="flex items-center justify-between bg-slate-800 p-3 rounded-lg">
                      <div className="flex items-center space-x-4">
                        <span className="font-mono">{slot.time}</span>
                        <span className="text-slate-400">Capacity: {slot.capacity}</span>
                      </div>
                      <div className="flex items-center space-x-2">
                        ...
                        <span className={`px-2 py-1 rounded-full text-xs ${
                          slot.status === 'available' ? 'bg-green-800 text-green-200' :
                          slot.status === 'cutoff' ? 'bg-yellow-800 text-yellow-200' :
              ...
              {bookings.length === 0 ? (
                ...
                <div className="mt-4 space-y-2">
                  {bookings.slice(0, 10).map((booking) => (
                    <div key={booking.id} className="flex items-center justify-between bg-slate-800 p-3 rounded-lg">
                      <div className="flex items-center space-x-4">
                        <span className="font-semibold">{booking.guestName}</span>
                        <span className="text-slate-400">Party of {booking.partySize}</span>
                        {booking.phone && <span className="text-slate-400">{booking.phone}</span>}
                      </div>
                      <div className="flex items-center space-x-2">
                        <select
                        ...
                        <span className={`px-2 py-1 rounded-full text-xs ${
                          ...
                          booking.status === 'held' ? 'bg-yellow-800 text-yellow-200' :
                          'bg-red-800 text-red-200'
                        }`}>
                          {booking.status}
                        </span>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setProfileData({ ...profileData, heroImageUrl: e.target.value })}
                  className="bg-slate-800 border border-slate-600 rounded-lg px-3 py-2 text-white w-full"
